...
class WindowsProject(Project):
    ...
    def package(self):
        ...
        # Copy GameNetworkingSockets and all dependencies from vcpkg (if available)
        vcpkg_root = os.environ.get("VCPKG_INSTALLATION_ROOT", "")
        if vcpkg_root:
            vcpkg_bin_dir = f"{vcpkg_root}/installed/x64-windows/bin"
            # Copy ALL DLLs from vcpkg to ensure version compatibility
            for file in glob.glob(f"{vcpkg_bin_dir}/*.dll"):
...
class MacProject(Project):
    ...
    def package(self):
        ...
        if vcpkg_root:
            ...
            # Copy ALL dylibs from vcpkg to ensure version compatibility
            for file in glob.glob(f"{vcpkg_lib_dir}/*.dylib"):
...
class LinuxProject(Project):
    ...
    def package(self):
        ...
        if vcpkg_root:
            ...
            vcpkg_lib_dir = f"{vcpkg_root}/installed/{triplet}/lib"
            # Copy ALL shared libraries from vcpkg to ensure version compatibility
            for file in glob.glob(f"{vcpkg_lib_dir}/*.so*"):
                shutil.copy(file, f"{appdir}/usr/lib", follow_symlinks=False)
